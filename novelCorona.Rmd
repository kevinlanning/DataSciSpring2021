...
Modified again Feb 5 because they changed the variable name for date again
...
for (i in (nsheets-24):(nsheets-1)) {
  j <- read_sheet (novelCoronaID, sheet = i) %>% 
        rename(`Last Update` = `Last Update`)
# if a variable doesn't exist in sheet, add it
  j[setdiff(varlist,names(j))] <- NA
  j %<>% select(varlist)
  coronaData <- rbind(coronaData, j) 
...
for (i in (nsheets):(nsheets)) {
  j <- read_sheet (novelCoronaID, sheet = i) %>% 
        rename(`Last Update` = `Date last updated`)
# if a variable doesn't exist in sheet, add it
  j[setdiff(varlist,names(j))] <- NA
  j %<>% select(varlist)
  coronaData <- rbind(coronaData, j) 
...
ggplot(coronaDataSimple) +
  geom_point(aes(x=reportDate, y=Deaths,
                       color = country))
```
